...
densityEUR <- density(EUR, kernel = 'epanechnikov', n = length(EUR))
probEUR <- densityEUR$y/sum(densityEUR$y)

plot(newEUR, probEUR, type = 'l', col = 'red')

newEUR <- seq(from = min(densityEUR$x), to = max(densityEUR$x), length.out = length(EUR))
replicatedSample <- sample(newEUR, size = length(EUR), replace = FALSE, prob = probEUR)
